...
const Precotizaciones = () => {
    ...
    useEffect(() => {
        const obtenerUltimoIdServicio = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/precotizaciones/ultimo-id-servicio');
    ...
    useEffect(() => {
        const fetchClienteData = async () => {
            if (id_cliente) {
                try {
                    const response = await fetch(`http://localhost:5000/api/catalogos/clientes/${id_cliente}`);
                    if (response.ok) {
                        const data = await response.json();
                        setClienteData(data);
                        setFormData(prev => ({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Verificar que el id_cliente esté presente
            if (!formData.id_cliente) {
                throw new Error('El ID de cliente es obligatorio');
            }
            // Verificar que el id_servicio esté presente
            ...
            const response = await fetch('http://localhost:5000/api/precotizaciones', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    id_servicio: String(nuevoIdServicio), // Usar el ID generado
                            ...
                            <div className="info-card">
                                ...
                                <div className="info-card-content">
                                    <div className="info-item">
                                        ...
                                        <span className="info-value">{id_cliente}</span>
                                    </div>
                                    <div className="info-item">
                                        <span className="info-label">Nombre:</span>
                                        <span className="info-value">{clienteData.nombre_cliente}</span>
                                    </div>
                                    <div className="info-item">
                                        <span className="info-label">Convenio:</span>
                                        <span className="info-value">{clienteData.tipo_convenio}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="info-card">
                                ...
                                <div className="info-card-content">
                                    <div className="info-item">
                                        ...
                                        <span className="info-value">{n_folio}</span>
                                    </div>
                                    <div className="info-item">
                                        <span className="info-label">ID Servicio:   </span>
                                        <span className="info-value">{nuevoIdServicio}</span>
                                    </div>
                                    <div className="info-item">
                                        <span className="info-label">Solicitante:   </span>
                                        <span className="info-value">{`${as_nombre || ''} ${ap_paterno || ''} ${ap_materno || ''}`}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="info-card">
                                ...
                                <div className="info-card-content">
                                    <div className="info-item">
                                        <span className="info-label">Kilómetros:    </span>
                                        <span className="info-value">{km} km</span>
                                    </div>
                                    <div className="info-item">
                                        <span className="info-label">Tiempo estimado:   </span>
                                        <span className="info-value">{minutos} minutos</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-columns">
                                <div className="form-column">
                                    <h3 className="column-title">Detalles del Servicio</h3>
                                ...
                                <div className="form-column">
                                    <h3 className="column-title">Costos Adicionales</h3>
                                    <div className="form-group">
                            ...
                            <div className="totals-section">
                                <h3 className="column-title">Totales</h3>
                                <div className="totals-grid">
                                    <div className="total-item">
                                        <span className="total-label">Total Neto (sin IVA):</span>
                                        <div className="total-value-container">
                                            <span className="currency">$</span>
                                            <span className="total-value">{formData.total}</span>
                                        </div>
                                    </div>
                                    <div className="total-item highlighted">
                                        <span className="total-label">Total con IVA (16%):</span>
                                        <div className="total-value-container">
                                            <span className="currency">$</span>
                                            <span className="total-value">{formData.total_con_iva}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
