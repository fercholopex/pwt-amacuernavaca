        ...
        return (
          <div className="action-buttons">
            <motion.button
              ...
              className="btn-action btn-iniciar"
        ...
        return (
          <div className="action-buttons">
            <motion.button
              ...
              className="btn-action btn-recordar"
              disabled={enviandoMensaje}
            >
            ...
            <motion.button
              ...
              className="btn-action btn-reasignar"
            >
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          ...
          className={`modal-overlay ${animate ? 'fade-in' : 'fade-out'}`}
        >
          <motion.div 
            ...
            className={`modal-content ${animate ? 'slide-in' : 'slide-out'}`}
          >
            ...
            <div className="tabs-container">
              <div 
                className={`tab ${activeTab === 'detalles' ? 'active' : ''}`}
                onClick={() => setActiveTab('detalles')}
              >
                <i className="fas fa-info-circle"></i> Detalles
              </div>
              <div 
                className={`tab ${activeTab === 'acciones' ? 'active' : ''}`}
                  ...
                  >
                    ...
                    {currentStep === 'asignar' && (
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="operator-section info-section"
                      >
                        <h3>
                    ...
                    {historialEventos.map((evento, index) => (
                      ...
                      >
                        ...
                        <div className="history-content">
                          <div className="history-title">{evento.titulo}</div>
                          <div className="history-date">{evento.fecha}</div>
                          <div className="history-description">{evento.descripcion}</div>
                        </div>
                      </motion.div>
                    ))}
                  </motion.div>
              ...
              >
                ...
                >
                  ...
                  <div className="confirmation-buttons">
                    ...
                    <button onClick={executeConfirmedAction} className="btn-confirm">
          ...
          <AnimatePresence>
            {showReasignarModal && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="reasignar-overlay"
              >
                <motion.div 
                  initial={{ scale: 0.8, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  exit={{ scale: 0.8, opacity: 0 }}
                  className="reasignar-dialog"
                >
                  ...
                  <div className="reasignar-buttons">
                    <button onClick={() => setShowReasignarModal(false)} className="btn-cancel">
                      <i className="fas fa-times"></i> Cancelar
                    </button>
                    <button 
                      onClick={() => {
  ...
  <div className="service-details">
    ...
    >
      ...
      <div className="info-grid">
        ...
        <div className="info-item">
          <label>Total:</label> 
          <span className="precio">${parseFloat(cotizacion.total_con_iva || '0').toFixed(2)}</span>
        </div>
      </div>
    </motion.div>
    <motion.div 
    ...
    >
      ...
      <div className="info-grid">
        ...
        <div className="info-item">
          <label>Placas:</label> 
          <span className="placas">{cotizacion.placas}</span>
        </div>
      </div>
    </motion.div>
    <motion.div 
      ...
      >
        ...
        <div className="info-grid">
          <div className="info-item full-width">
            ...
            <span className="poliza-numero">{cotizacion.n_poliza}</span>
          </div>
        </div>
      </motion.div>
    )}
    ...
    {!isCostConfirmed && (
      <div className="cost-actions">
        <motion.button
          ...
          className="btn-action btn-confirmar-detalles"
        >
...
const ConfirmPoliza: React.FC<ConfirmPolizaProps> = ({
  ...
  <motion.div 
    ...
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay: 0.2 }}
    className="confirm-poliza"
  >
    <h3>
      <i className="fas fa-file-alt"></i> Póliza
    </h3>
    <div className="poliza-inputs">
      ...
      <div className="input-group">
        ...
        <div className="input-with-icon">
          ...
          <input
            ...
            disabled={isPolizaConfirmed}
            className={confirmPoliza && poliza !== confirmPoliza ? 'error' : ''}
          />
        </div>
      </div>
    </div>
    {!isPolizaConfirmed && (
      <div className="poliza-actions">
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={handleConfirmPoliza}
          className="btn-action btn-confirmar-poliza"
